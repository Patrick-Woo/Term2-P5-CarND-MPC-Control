...
//PM... Add new constant values like reference velocity and so forth
...
// PM...using the classroom quiz's code as it is easy to be read and understood.
    ...
    // the Solver function below.
	  ...
	  // The cost is stored is the first element of `fg`.
	      ...
	      for (int t = 0; t < N; t++) {
	        ...
	        fg[0] += cost_eps * CppAD::pow(vars[epsi_start + t], 2);
	        fg[0] += cost_v * CppAD::pow(vars[v_start + t] - ref_v, 2);
	      }

	      // Minimize the use of actuators.
	      for (int t = 0; t < N - 1; t++) {
	        fg[0] += cost_current_delta*CppAD::pow(vars[delta_start + t], 2);
	        fg[0] += cost_current_a*CppAD::pow(vars[a_start + t], 2);
	        // try adding penalty for speed + steer
	        fg[0] += cost_v_product_delta*CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2);
	      }

	      // Minimize the value gap between sequential actuations.
	      for (int t = 0; t < N - 2; t++) {
	        fg[0] += cost_diff_delta*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
	        fg[0] += cost_diff_delta*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
	      }
	      ...
	      for (int t = 1; t < N; t++) {
	        ...
	        // PM...as we consider it is a 3 order curves, we don't use one order straight line.
	       // AD<double> f0 = coeffs[0] + coeffs[1] * x0;
	       // AD<double> psides0 = CppAD::atan(coeffs[1]);
	       //f'(x) to calculate the epsi
